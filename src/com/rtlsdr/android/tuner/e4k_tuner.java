...
public class e4k_tuner implements RtlSdr_tuner_iface {
	...
	private final byte [] if_stage1_gain = {
		-3, 6
	};
	...
	private final int [] if_stage_gain_len = {
		0,
		if_stage1_gain.length,
		if_stage23_gain.length,
		if_stage23_gain.length,
		if_stage4_gain.length,
	...
	private  int MHZ(int x)	{return((x)*1000*1000);	}
	
	private  int KHZ(int x){return((x)*1000);}
	...
	{
		return SdrUSBDriver.rtlsdr_i2c_write_reg((byte)E4K_I2C_ADDR,(char) reg, (char)val);
	}
	...
	{
		return e4k_reg_set_mask((byte)E4K_REG_FILT3, (char)E4K_FILT3_DISABLE,(char)( on ? 0 : E4K_FILT3_DISABLE));
	}
	...
	int e4k_init()
	{
		...
		e4k_reg_write((byte) E4K_REG_MASTER1,(byte)(
				E4K_MASTER1_RESET |
	...
	/*#if 0
		...
		e4k_reg_set_mask((byte) E4K_REG_AGC1,(char) E4K_AGC1_MOD_MASK, (char)E4K_AGC_MOD_SERIAL);

		/* Set Mixer Gain Control to manual */
		e4k_reg_set_mask((byte) E4K_REG_AGC7,(char) E4K_AGC7_MIX_GAIN_AUTO, (char)0);

	/*
	#if 0/* Enable LNA Gain enhancement * /
	...
	private  int e4k_enable_manual_gain( boolean manual)
	{
		if (manual) {
			...
			e4k_reg_set_mask((byte) E4K_REG_AGC1,(char) E4K_AGC1_MOD_MASK,(char) E4K_AGC_MOD_IF_SERIAL_LNA_AUTON);
			/* Set Mixer Gain Control to auto */
			e4k_reg_set_mask((byte) E4K_REG_AGC7,(char) E4K_AGC7_MIX_GAIN_AUTO,(char) 1);
