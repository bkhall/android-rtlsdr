...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
	 ...
	 * 
	 * @param src
	 *            The source buffer (which contains status bytes)
	 * @param dest
	 *            The destination buffer to write the status bytes into (can be
	 *            src)
	 * @param totalBytesRead
	 *            Number of bytes read to src
	 * @param maxPacketSize
	 *            The USB endpoint max packet size
	 * @return The number of payload bytes
	 */
	private final int filterStatusBytes(byte[] src, byte[] dest,
			int totalBytesRead, int maxPacketSize) {
		final int packetsCount = totalBytesRead / maxPacketSize + 1;
		for (int packetIdx = 0; packetIdx < packetsCount; ++packetIdx) {
			final int count = (packetIdx == (packetsCount - 1)) ? (totalBytesRead % maxPacketSize)
					- MODEM_STATUS_HEADER_LENGTH
					: maxPacketSize - MODEM_STATUS_HEADER_LENGTH;
			if (count > 0) {
				System.arraycopy(src, packetIdx * maxPacketSize
						+ MODEM_STATUS_HEADER_LENGTH, dest, packetIdx
						* (maxPacketSize - MODEM_STATUS_HEADER_LENGTH), count);
			}
		}
	...
	public int read(byte[] dest, int timeoutMillis) throws IOException {
		...
		if (ENABLE_ASYNC_READS) {
			...
			synchronized (mReadBufferLock) {
				...
				return filterStatusBytes(mReadBuffer, dest, totalBytesRead,
	...
	public void setParameters(int baudRate, int dataBits, int stopBits,
		...
		case PARITY_SPACE:
			...
			break;
		default:
			throw new IllegalArgumentException("Unknown parity value: "
					+ parity);
		}
		...
		case STOPBITS_2:
			...
			break;
		default:
			throw new IllegalArgumentException("Unknown stopBits value: "
					+ stopBits);
		}

		int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
				SIO_SET_DATA_REQUEST, config, 0 /* index */, null, 0,
				USB_WRITE_TIMEOUT_MILLIS);
		if (result != 0) {
